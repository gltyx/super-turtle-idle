...
var player = {
    coins: { amount: 1, second: 0, perClick: 100, upgrades: 1, upgradesClick: 0 },
...
function rememberCategory(){
   ...
   if (settings.currentCategory === "jobContainer") {did("jobTab").classList.add("botonLateralActivo"); did("jobTitle").style.display = "flex";}
...
function save() {
  ...
  saveData.savedBuffTime = {}; for (const i in buffs) { saveData.savedBuffTime[i] = buffs[i].left;}
  saveData.savedBuffActive = {}; for (const i in buffs) { saveData.savedBuffActive[i] = buffs[i].active;}
...
function load() {
  ...
  if (datosGuardados) { //checks if savedata available
    ...
    for (const i in parsedData.savedBuffTime) { buffs[i].left = parsedData.savedBuffTime[i];}
    for (const i in parsedData.savedBuffActive) { buffs[i].active = parsedData.savedBuffActive[i];}  
